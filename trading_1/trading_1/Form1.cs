        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int iNYSE = 0; iNYSE < NYSEsymbol.Count; iNYSE++)
            {
                ...
                try
                {
                    ...
                    string read_file_name = "\\" + NYSEsymbol[iNYSE] + "testOut1.csv";
                ...
                if(excSkip == false)
                {
                    ...
                    #region Plot data to chart
                    
                    int numDaysToDisplay = 365 * 1;
                    if(plotting==true)
                    {
                        for (int i = 0; i < close.Count; i++)
                        {
                            // Plot close date
                            if (i > close.Count - numDaysToDisplay) // only plot the last 60 days
                            {
                                chart1.Series[0].Points.AddXY(i, close[i]);
                                //chart1.ChartAreas[0].AxisY.Minimum = close.Min(x => x.Value);
                                chart1.ChartAreas[0].AxisY.Minimum = 0;// close[close.Count - 600];
                            }
                        }
                    }
                    #endregion Plot data to chart
                    ...
                    for (int longDay = 1; longDay < 4; longDay++) // up to 90 days moving average
                    {
                        pctRtrn = 0;
                        buyTmp = 0;
                        bought = false;
                        ...
                        for (int shortDay = 2; shortDay < 4; shortDay++)
                        {
                            pctRtrn = 0;
                            buyTmp = 0;
                            bought = false;
                            ...
                            for (int sd = 0; sd < close.Count; sd++)
                            {
                                if (sd + 1 > shortDay)
                                {
                                    ...
                                    if (plotting == true)
                                    {
                                        if (sd > close.Count - numDaysToDisplay) // only plot the last 60 days
                                        {
                                            chart1.Series[1].Points.AddXY(sd, smaTmp);
                                ...
                                if (ld + 1 > longDay)
                                {
                                     ...
                                     if (plotting == true)
                                     {
                                         if (ld > close.Count - numDaysToDisplay) // only plot the last 60 days
                                         {
                                             chart1.Series[2].Points.AddXY(ld, lmaTmp);
                                         }
                                     }
                                 }

                                    //for (int bd = 0; bd < close.Count; bd++)
                                    //{
                                    //    if (bd + 1 > bullBearDay)
                                    //    {
                                    //        lmaTmp = 0;
                                    //        for (int j = 0; j < bullBearDay; j++)
                                    //        {
                                    //            lmaTmp = bbTmp + close[bd - bullBearDay + j];
                                    //        }
                                    //        bbTmp = bbTmp / bullBearDay;
                                    //        shortSMA.Add(shortSMA.Count, bbTmp);
                                    //        if (bd > close.Count - numDaysToDisplay) // only plot the last 60 days
                                    //        {
                                    //            chart1.Series[3].Points.AddXY(bd, bbTmp);
                                    //        }
                                    //    }
                                    //}

                                    // Calculate the difference between short and long day moving average and determine buy and sell
                                    //double pctRtrn = 0;
                                    //double buyTmp = 0;
                                    //bool bought = false;
                                    //bool sold = false;
                                    //for (int bs = 0; bs < close.Count-1; bs++)
                                    //{
                                    int bs = ld;
                                    if (bs < close.Count - 1)
                                    {
                            ...
                            //textBox2.Text = pctRtrn.ToString() + "\r\n";
                            //textBox2.Text += pctRtrn.ToString() + Environment.NewLine;

                            #region write sma/lma and pct to file for each symbol
                            StreamWriter objWriter;
                            string write_file_name = "\\" + NYSEsymbol[iNYSE] + "_Optimize_Out1.csv";
